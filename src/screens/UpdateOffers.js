...
const UpdateOffers = ({ navigation, route }) => {
    const defaultImage =
    ...
    const getOffersByID = (offerID) => {
        axios
            .get(`${AxiosURL}/customer/getOffersById/${offerID}/`)
            .then((response) => {
                if (response.data.status === 200) {
                    setOfferDetails(response.data.response.offer);
                } else {
                    ToastAndroid.show(
    ...
    const onUpdateOfferPress = async () => {
        startLoading();
        ...
        let offer_id = offerDetails.offer_id;
        let post_time = offerDetails.post_time;
        let likes = offerDetails.likes;

        console.log(offerDetails);

        if (offer_title && details) {
            if (image) {
                try {
            ...
            console.log(updateOfferDetails);

            axios
                .put(
                    `${AxiosURL}/seller/updateOffer/${sellerID}/${offer_id}`,
                    updateOfferDetails
                )
                .then((response) => {
                    if (response.data.status === 200) {
    ...
    useEffect(() => {
        (async () => {
            ...
            try {
                getOffersByID(route.params.offerID);
            } catch (error) {
                console.log(error);
            }
        })();
        ...
        >
            ...
            <View style={styles.header}>
                ...
                <Animatable.View animation="fadeInLeftBig" duration={1500}>
                    <Text style={styles.headerText}>Update Offer</Text>
                </Animatable.View>
            </View>
            <Animatable.View
                    ...
                    <ScrollView showsVerticalScrollIndicator={false}>
                        <View>
                            ...
                            <Text style={{ color: colors.text }}>
                                NOTE: You can set offer period to maximum 7 days
                                only. After 7 days offer will be removed
                                automatically from homescreen.
                            </Text>
